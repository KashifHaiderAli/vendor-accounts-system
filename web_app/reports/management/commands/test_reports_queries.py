...
class Command(BaseCommand):
    ...
    def first_id(self, table, company_id, branch_id, allow_null_branch=False):
        if not report_utils.table_exists(table):
            return ""
        branch_clause = "(branch_id=%s OR branch_id IS NULL)" if allow_null_branch else "branch_id=%s"
        with connection.cursor() as cursor:
            cursor.execute(

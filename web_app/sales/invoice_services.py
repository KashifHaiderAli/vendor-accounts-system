...
def calculate_items(items, company_id, branch_id):
    ...
    for raw in items:
        ...
        tax_percent, errors["tax_percent"] = validators.validate_percentage(raw.get("tax_percent") or default_tax(company_id, branch_id), "Tax Percent")
...
def get_invoice(company_id, branch_id, invoice_id):
    with connection.cursor() as cursor:
        cursor.execute(
            """
            SELECT si.*, c.company_name, c.address AS customer_address, c.phone AS customer_phone,

...
def validate_contract(data, company_id, branch_id, contract_id=None):
    ...
    cleaned["tax_applicable"] = validators.normalize_bool(data.get("tax_applicable"))
...
def generate_invoice_from_contract(request, contract):
    ...
    tax = get_tax_settings(company_id, branch_id)
    invoice_type = "tax_invoice" if int(contract.get("tax_applicable") or 0) == 1 else "cash_memo"

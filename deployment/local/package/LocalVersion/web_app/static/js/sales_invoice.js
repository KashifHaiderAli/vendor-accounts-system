document.addEventListener("DOMContentLoaded", () => {
    ...
    const recalcRow = (row) => {
        ...
        const taxAmount = row.querySelector("[name='tax_amount_display[]']");
        if (taxAmount) taxAmount.value = money(tax);
    ...
    const bindRow = (row) => {
        ...
        row.querySelector(".item-select")?.addEventListener("change", (event) => {
            ...
            const taxInput = row.querySelector("[name='tax_percent[]']");
            if (taxInput) taxInput.value = money(Number.parseFloat(option.dataset.tax || "0"));
            recalcTotals();
        });
    };

...
def validate_and_calculate(data, company_id, branch_id, voucher_id=None):
    ...
    tax_input = 0 if not is_tax_enabled(company_id=company_id) else data.get("tax_percent")
    tax_percent, errors["tax_percent"] = validators.validate_percentage(tax_input, "Tax Percent")
    cleaned["amount"] = amount or Decimal("0.00")
    cleaned["tax_percent"] = Decimal("0.00") if not is_tax_enabled(company_id=company_id) else (tax_percent or Decimal("0.00"))

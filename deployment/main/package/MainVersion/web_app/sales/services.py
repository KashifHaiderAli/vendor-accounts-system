...
def get_quotation(company_id, branch_id, quotation_id):
    with connection.cursor() as cursor:
        cursor.execute(
            """
            SELECT q.*, c.customer_code AS master_customer_code, c.company_name AS master_customer_name, c.address AS master_customer_address,
                   c.phone AS master_customer_phone, c.mobile AS master_customer_mobile, c.email AS master_customer_email,
                   c.contact_person AS master_contact_person, c.ntn AS master_customer_ntn, c.strn AS master_customer_strn,
...
def apply_customer_display_fields(quotation):
    ...
    quotation["display_customer_code"] = quotation.get("master_customer_code") or ""
    quotation["display_contact_person"] = quotation.get("contact_person") or quotation.get("master_contact_person") or ""

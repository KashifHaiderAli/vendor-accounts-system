...
"""SQLite schema for the DB App.

The schema includes return transaction support through sales_returns,
sales_return_items, purchase_returns, and purchase_return_items. These tables
mirror the invoice and purchase patterns so the later web app can post return
journal entries and report by company, branch, customer/supplier, status, and date.
"""
...
INDEX_STATEMENTS = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_item_services_company_branch ON item_services(company_id, branch_id)",
    "CREATE INDEX IF NOT EXISTS idx_cash_bank_accounts_account_id ON cash_bank_accounts(account_id)",
    "CREATE INDEX IF NOT EXISTS idx_expense_heads_account_id ON expense_heads(account_id)",
    "CREATE INDEX IF NOT EXISTS idx_quotations_company_branch ON quotations(company_id, branch_id)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_confirmations_date ON customer_confirmations(confirmation_date)",
    "CREATE INDEX IF NOT EXISTS idx_confirmations_no ON customer_confirmations(confirmation_no)",
    "CREATE INDEX IF NOT EXISTS idx_confirmations_customer_id ON customer_confirmations(customer_id)",
    "CREATE INDEX IF NOT EXISTS idx_confirmations_status ON customer_confirmations(status)",
    "CREATE INDEX IF NOT EXISTS idx_delivery_challans_date ON delivery_challans(dc_date)",
    "CREATE INDEX IF NOT EXISTS idx_delivery_challans_no ON delivery_challans(dc_no)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_sales_returns_company_branch ON sales_returns(company_id, branch_id)",
    "CREATE INDEX IF NOT EXISTS idx_sales_returns_date ON sales_returns(return_date)",
    "CREATE INDEX IF NOT EXISTS idx_sales_returns_no ON sales_returns(sales_return_no)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_supplier_purchases_no ON supplier_purchases(purchase_no)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_purchase_returns_company_branch ON purchase_returns(company_id, branch_id)",
    "CREATE INDEX IF NOT EXISTS idx_purchase_returns_date ON purchase_returns(return_date)",
    "CREATE INDEX IF NOT EXISTS idx_purchase_returns_no ON purchase_returns(purchase_return_no)",
    "CREATE INDEX IF NOT EXISTS idx_purchase_returns_supplier_id ON purchase_returns(supplier_id)",
    "CREATE INDEX IF NOT EXISTS idx_purchase_returns_purchase_id ON purchase_returns(supplier_purchase_id)",
    ...
    "CREATE INDEX IF NOT EXISTS idx_license_records_company_branch ON license_records(company_id, branch_id)",
    "CREATE INDEX IF NOT EXISTS idx_user_activity_datetime ON user_activity_log(activity_datetime)",
]

ALL_STATEMENTS = SCHEMA_STATEMENTS + INDEX_STATEMENTS
...
def create_schema(connection) -> None:
    # CREATE TABLE handles new databases. The lightweight column migration keeps
    # existing local SQLite databases aligned when new setup fields are added.
...
def table_names() -> list[str]:
    ...
    for statement in SCHEMA_STATEMENTS:
        line = statement.strip().splitlines()[0]
        names.append(line.split("IF NOT EXISTS", 1)[1].strip().split(" ", 1)[0])
    return names
